...
def load_forces(fname):
    d     = np.load(f"{data_dir}/{fname}")
    f_key = "forces_mace_flat" if "forces_mace_flat" in d else "forces_mlip_flat"
    f_dft  = d["forces_dft_flat"].flatten() * ev_kjmol
    f_mlip = d[f_key].flatten()             * ev_kjmol
    return f_dft, f_mlip
...
for fname, label in mlip_info:
    f_dft, f_mlip = load_forces(fname)
    all_data.append((label, f_dft, f_mlip))
    print(f"{label}: loaded {len(f_dft):,} force components")

lo, hi   = -520, 520
xticks   = np.array([-500, -250, 0, 250, 500])
...
for idx, (label, f_dft, f_mlip) in enumerate(all_data):
    ...
    ax.set_xlabel(
        r"$\mathrm{DFT\ Force}\ (\mathrm{kJ\,mol}^{-1}\,\mathrm{\AA}^{-1})$",
        fontsize=26)
    if col == 0:
        ax.set_ylabel(
            r"$\mathrm{MLIP\ Force}\ (\mathrm{kJ\,mol}^{-1}\,\mathrm{\AA}^{-1})$",
            fontsize=26)
    else:
        ax.tick_params(labelleft=False)

    mae = compute_mae(f_dft, f_mlip)
    ax.text(
        0.42, 0.87,
        rf"$\mathrm{{MAE}} = {mae:.2f}\ \mathrm{{kJ\,mol}}^{{-1}}\,\mathrm{{\AA}}^{{-1}}$",
...
out_path = f"{out_dir}/force_parity_metafigure.pdf"
